...
def extract_text(file_bytes: bytes) -> str:
    # Попытка 1: pypdf — самый быстрый, останавливаемся если результат хороший
    try:
        import pypdf
        ...
        t = "\n".join(pages).strip()
        if len(t) > 300:
            logger.info(f"pypdf OK: {len(t)} chars")
            return t
        else:
            logger.warning(f"pypdf: too short ({len(t)} chars), trying next")
    except Exception as e:
        logger.warning(f"pypdf failed: {e}")

    # Попытка 2: pdfminer.six — лучше читает сложные PDF
    try:
        from pdfminer.high_level import extract_text as pm_extract
        t = (pm_extract(io.BytesIO(file_bytes)) or "").strip()
        if len(t) > 300:
            logger.info(f"pdfminer OK: {len(t)} chars")
            return t
        else:
            logger.warning(f"pdfminer: too short ({len(t)} chars), trying next")
    except ImportError:
        logger.warning("pdfminer not installed")
    except Exception as e:
        logger.warning(f"pdfminer failed: {e}")

    # Попытка 3: pdfplumber — последний резерв
    try:
        import pdfplumber
        pages = []
        ...
        t = "\n".join(pages).strip()
        if len(t) > 100:
            logger.info(f"pdfplumber OK: {len(t)} chars")
            return t
        else:
            logger.warning(f"pdfplumber: too short ({len(t)} chars)")
    except ImportError:
...
def detect_type(text: str) -> str:
    ...
    gkb_strong = [
        ...
        # fcbk (Первое кредитное бюро — другой формат)
        "id субъекта",
        "персональный - физическое лицо",
        "бдки",
        "данные о субъекте кредитной истории",
...
def parse_gkb(text: str) -> dict:
    ...
    if is_kz:
        ...
        fam = re.search(r"Тегі\s*:?\s*" + f"({_KZ_NAME})" + _KZ_NEXT, text, re.I)
        nm  = re.search(r"\bАты\s*:?\s*" + f"({_KZ_NAME})" + _KZ_NEXT_ATY, text, re.I)
        oth = re.search(r"Әкесінің\s*аты\s*:?\s*" + f"({_KZ_NAME})" + _KZ_NEXT_OTH, text, re.I)
        if fam and nm:
    ...
    if is_kz:
        ...
        #
        # ВАЖНО 2: маркер может быть как слитым так и с пробелами,
        # поэтому проверяем оба варианта для каждого слова.
        ...
        start_markers_kz = [
            ...
            # частичный вариант с пробелами
            "қ о л д а н ы с т а ғ ы",  # на случай поиздевательских PDF
        ...
        for marker in start_markers_kz:
        ...
        # Если вообще ничего не нашли — начинаем с 0
        if active_start == -1:
            active_start = 0
    ...
    # ══════════════════════════════════════════════════════════
    # ОПРЕДЕЛЯЕМ ФОРМАТ: fcbk (Контракт N) или ГКБ (Обязательство N)
    # ══════════════════════════════════════════════════════════
    is_fcbk = (
        "id субъекта" in t_low
        or "источник информации (кредитор)" in t_low
        or "действующие договора" in t_low
    )

    if is_fcbk:
        # ── fcbk формат: ФИО из таблицы ─────────────────────────
        if not d.get("full_name"):
            m = re.search(r"ФИО\s+([А-ЯЁ][А-ЯЁа-яё\s]+?)(?:\n|Фамилия|Резидент|Документы)", text, re.I)
            if m:
                d["full_name"] = _clean(m.group(1))
        if not d.get("full_name"):
            # Ищем строку с 3 словами заглавными буквами
            m = re.search(r"([А-ЯЁ]{2,}\s+[А-ЯЁ]{2,}\s+[А-ЯЁ]{2,})", text)
            if m:
                d["full_name"] = _clean(m.group(1))

        # ── fcbk: Активные контракты (до «Завершенные договора») ─
        end_marker_fcbk = t_low.find("завершенные договора")
        if end_marker_fcbk == -1:
            end_marker_fcbk = t_low.find("завершённые договора")
        fcbk_active_text = text[:end_marker_fcbk] if end_marker_fcbk != -1 else text

        # ── fcbk: блоки по «Контракт N» ───────────────────────────
        contract_re = re.compile(r"Контракт\s+(\d+)", re.IGNORECASE)
        contract_matches = list(contract_re.finditer(fcbk_active_text))

        for i, m_obj in enumerate(contract_matches):
            start = m_obj.start()
            end = contract_matches[i + 1].start() if i + 1 < len(contract_matches) else len(fcbk_active_text)
            block = fcbk_active_text[start:end]
            cr = _parse_fcbk_block(block, BANKS)
            if cr:
                d["credits"].append(cr)
...
def _parse_fcbk_block(block: str, BANKS: dict) -> dict | None:
    """
    Парсит один блок «Контракт N» из отчёта fcbk (Первое кредитное бюро).

    Формат fcbk:
      Источник информации (Кредитор): АО "Kaspi Bank"
      Номер договора: R3152246-003
      Статус договора: Просроченная задолженность свыше 360 дней
      Общая сумма кредита/валюта: 2 900 000,00 KZT
      Сумма просроченных взносов: 2 752 988,36 KZT
      Количество дней просрочки: 385
      Непогашенная сумма по кредиту: 1 219 155,80 KZT
    """
    cr = {}
    b_low = block.lower()

    # ── Статус — пропускаем завершённые ──────────────────────
    status_m = re.search(r"Статус\s+договора[:\s]*(.+?)(?:\n|Классификация)", block, re.I)
    if status_m:
        status_val = status_m.group(1).strip().lower()
        closed_kw = ["стандартные кредиты", "погашен", "завершён", "завершен", "закрыт"]
        if any(k in status_val for k in closed_kw):
            return None
        cr["status"] = status_m.group(1).strip()

    # ── Кредитор ─────────────────────────────────────────────
    m = re.search(r"Источник\s+информации\s*\(Кредитор\)\s*:\s*(.+?)(?:\n|БИН)", block, re.I | re.DOTALL)
    if not m:
        m = re.search(r"Источник\s+информации\s*\(Кредитор\)\s*:\s*(.+?)(?=\nБИН|\nТип|$)", block, re.I | re.DOTALL)
    if not m:
        return None
    ...
    # ── Номер договора ────────────────────────────────────────
    m = re.search(r"Номер\s+договора\s*:\s*(\S+)", block, re.I)
    if m:
        cr["contract_number"] = m.group(1).strip()[:40]

    # ── Дата выдачи ───────────────────────────────────────────
    m = re.search(r"Дата\s+фактической\s+выдачи\s*:\s*(\d{2}\.\d{2}\.\d{4})", block, re.I)
    if not m:
        m = re.search(r"Дата\s+начала\s+срока\s+действия\s+договора\s*:\s*(\d{2}\.\d{2}\.\d{4})", block, re.I)
    if m:
        cr["contract_date"] = m.group(1)

    # ── Общая сумма займа ─────────────────────────────────────
    for pat in [
        r"Общая\s+сумма\s+кредита/валюта\s*:\s*([\d\s\xa0,]+\.?\d*)\s*KZT",
        r"Сумма\s+кредитного\s+лимита\s*:\s*([\d\s\xa0,]+\.?\d*)\s*KZT",
        r"Общая\s+сумма\s+кредита\s*/\s*валюта\s*:\s*([\d\s\xa0,]+\.?\d*)\s*KZT",
    ]:
        m = re.search(pat, block, re.I)
        if m:
            v = _parse_amount(m.group(1))
            if v and 100 < v < 2_000_000_000:
                cr["loan_amount"] = v
                break

    # ── Непогашенная сумма (остаток) ──────────────────────────
    for pat in [
        r"Непогашенная\s+сумма\s+по\s+кредиту\s*:\s*([\d\s\xa0,]+\.?\d*)\s*KZT",
        r"Использованная\s+сумма\s*\(подлежащая\s+погашению\)\s*:\s*([\d\s\xa0,]+\.?\d*)\s*KZT",
    ]:
        m = re.search(pat, block, re.I)
        if m:
            v = _parse_amount(m.group(1))
            if v is not None and v >= 0:
                cr["remaining_amount"] = v
                break

    # ── Просроченная сумма ────────────────────────────────────
    for pat in [
        r"Сумма\s+просроченных\s+взносов\s*:\s*([\d\s\xa0,]+\.?\d*)\s*KZT",
        r"Сумма\s+просроченн\w+\s+взносов\s*:\s*([\d\s\xa0,]+\.?\d*)\s*KZT",
    ]:
        m = re.search(pat, block, re.I)
        if m:
            v = _parse_amount(m.group(1))
    ...
    # ── Количество дней просрочки ─────────────────────────────
    m = re.search(r"Количество\s+дней\s+просрочки\s*:\s*(\d+)", block, re.I)
    if m:
        cr["overdue_days"] = int(m.group(1))

    # Если нет ни суммы ни дней — скорее всего нет долга
    if not cr.get("loan_amount") and not cr.get("overdue_amount"):
        return None
...
def _clean_bank_name(name: str) -> str:
    """Убираем спецсимволы Markdown и разделяем слипшиеся слова (строчная→заглавная)."""
    name = name.replace('"', '').replace('*', '').replace('_', '').replace('`', '')
    # Только строчная → заглавная, чтобы не трогать аббревиатуры (ТОО, АО, МФО)
    name = re.sub(r'([а-яёa-zқғүұөәіңһ])([А-ЯҚҒҮҰӨӘІҢҺA-Z])', r'\1 \2', name)
...
def format_gkb(d: dict) -> str:
    ...
    for i, cr in enumerate(credits, 1):
        bank_name = _clean_bank_name(cr['bank'])
        lines.append("━━━━━━━━━━━━━━━━━━")
        ...
        # Предупреждение о нуле прямо в списке
        days = cr.get("overdue_days")
        overdue = cr.get("overdue_amount", 0) or 0
        if days is not None and int(days) == 0 and int(str(overdue).replace(",","").replace(" ","")) == 0:
            lines.append("   ⚠️ _Просрочка 0 — возможно ошибка ГКБ или кредит обслуживается_")
        if cr.get("anomaly"):
...
def parse_in(text: str) -> dict:
    ...
    # ФИО должника — поддержка русского и казахского алфавита
    _NAME_CHAR = r"[А-ЯЁа-яёӘәІіҢңҒғҮүҰұҚқӨөҺһA-Za-z]"
    _NAME_WORD = _NAME_CHAR + r"+"
    _FULL_NAME = _NAME_WORD + r"\s+" + _NAME_WORD + r"(?:\s+" + _NAME_WORD + r")?"

    for pat in [
        # Казахский/русский: взыскать ... с ФИО, дата
        r"взыскать[^с]{0,50}с\s+(" + _FULL_NAME + r"),?\s*\d{2}\.\d{2}",
        # Просто: с ФИО, дата
        r"с\s+(" + _FULL_NAME + r"),?\s*\d{2}\.\d{2}\.\d{4}",
    ]:
    ...
    _KZ_RU_WORD = r"[А-ЯЁа-яёӘәІіҢңҒғҮүҰұҚқӨөҺһ\-]+"
    m = re.search(r"Я,?\s+(" + _KZ_RU_WORD + r"\s+" + _KZ_RU_WORD + r"\s+" + _KZ_RU_WORD + r"),?\s+нотариус", text)
    if not m:
        m = re.search(r"Нотариус\s+([А-ЯЁа-яёӘәІіҢңҒғҮүҰұҚқӨөҺһ][А-ЯЁа-яёӘәІіҢңҒғҮүҰұҚқӨөҺһ\s]+?)(?:\n|$)", text)
    if m:
