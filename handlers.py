...
ПОТОКИ:
  ...
  AI-юрист         → чат с Claude (Anthropic API)
"""
import io, logging
...
from pdf_parser import extract_text, normalize_text, parse_document, format_gkb, format_in, format_il, _clean_bank_name
...
from ai_lawyer import ask_ai_lawyer
...
# Кнопки выхода из чата с AI
AI_END_RU = "🔚 Завершить чат"
AI_END_KZ = "🔚 Чатты аяқтау"
AI_END_BUTTONS = [AI_END_RU, AI_END_KZ]
...
def kb_ai_chat(lang):
    """Клавиатура внутри чата с AI-юристом."""
    end_btn = AI_END_RU if lang == "ru" else AI_END_KZ
    home_btn = HOME_RU if lang == "ru" else HOME_KZ
    return ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text=end_btn)],
...
def kb_gkb(credits: list, lang: str):
    ...
    seen = set()
    for cr in credits:
        bank_name = _clean_bank_name(cr['bank'])[:38]
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if not await is_subscribed(message.bot, message.from_user.id):
        await message.answer(
            ...
            "∞ *Реструктуризация* · 📝 *Отмена ИН* · ⚖️ *Отмена суда* · 🤖 *AI-юрист*\n\n"
            "🇰🇿 Мен — *AMANAT* партиясының *Qaryzsyz Qogam* жобасының боты.\n"
            "Заңдық өтініштерді *тегін* жасауға көмектесемін:\n"
            "∞ *Қайта құрылымдау* · 📝 *ИН тоқтату* · ⚖️ *Сот шешімін тоқтату* · 🤖 *AI-заңгер*\n\n"
...
# ══════════════════════════════════════════════════════════════
# Кнопка "🔚 Завершить чат" — выход из AI-юриста в меню
# ══════════════════════════════════════════════════════════════

@router.message(F.text.in_(AI_END_BUTTONS))
async def ai_end_chat(message: Message, state: FSMContext):
    data = await state.get_data()
    lang = gl(data)
    await state.clear()
    await state.update_data(lang=lang)
    await state.set_state(Form.menu)
    if lang == "ru":
        text = "✅ *Чат с AI-юристом завершён.*\n\nЕсли есть ещё вопросы — заходите снова!"
    else:
        text = "✅ *AI-заңгермен чат аяқталды.*\n\nТағы сұрақтарыңыз болса — қайта кіріңіз!"
    await message.answer(text, parse_mode="Markdown", reply_markup=kb_menu(lang))
...
async def menu_choice(message: Message, state: FSMContext):
    ...
    # ── AI-юрист — переход в чат-режим ───────────────────────
    if doc_type == "ai_lawyer":
        await state.update_data(doc_type=doc_type, ai_history=[])
        await state.set_state(Form.ai_chat)
        if lang == "ru":
            intro = (
                "🤖 *AI-юрист проекта Qaryzsyz Qogam*\n\n"
                "Я помогу разобраться с вопросами по:\n"
                "• Реструктуризации долгов\n"
                "• Отмене исполнительной надписи\n"
                "• Отмене решения суда\n"
                "• Банкротству физических лиц\n"
                "• Работе с коллекторами и ЧСИ\n"
                "• Кредитной истории и ГКБ\n\n"
                "✏️ *Задайте свой вопрос — я отвечу простым языком.*\n\n"
                "⚠️ Это информационная консультация, не индивидуальный правовой совет. "
                "В сложных случаях звоните: *8-700-855-85-85*"
            )
        else:
            intro = (
                "🤖 *Qaryzsyz Qogam жобасының AI-заңгері*\n\n"
                "Мен мынадай сұрақтарға көмектесемін:\n"
                "• Қарыздарды қайта құрылымдау\n"
                "• Атқарушылық жазбаны күшін жою\n"
                "• Сот шешімін күшін жою\n"
                "• Жеке тұлғалардың банкроттығы\n"
                "• Коллекторлар мен ЖСО-мен жұмыс\n"
                "• Кредиттік тарих және МКБ\n\n"
                "✏️ *Сұрағыңызды қойыңыз — қарапайым тілмен жауап беремін.*\n\n"
                "⚠️ Бұл — ақпараттық кеңес, жеке құқықтық кеңес емес. "
                "Күрделі жағдайларда қоңырау шалыңыз: *8-700-855-85-85*"
            )
        await message.answer(intro, parse_mode="Markdown", reply_markup=kb_ai_chat(lang))
        return
...
# ══════════════════════════════════════════════════════════════
# AI-ЮРИСТ — обработка сообщений в чате
# ══════════════════════════════════════════════════════════════
...
async def ai_chat_handler(message: Message, state: FSMContext):
    ...
    # Без текста (фото, видео и т.п.) — попросим написать текстом
    if not text:
        await message.answer(
            "✏️ Напишите ваш вопрос текстом."
            if lang == "ru" else "✏️ Сұрағыңызды жазыңыз."
        )
        return
    ...
    answer, new_history = await ask_ai_lawyer(text, history, lang=lang)
    await state.update_data(ai_history=new_history)

    # Логируем обращение к AI
    log_event(message.from_user.id, "ai_lawyer_msg")

    # Кнопки чата (всегда видны под последним ответом)
    kb = kb_ai_chat(lang)

    # Telegram имеет лимит 4096 символов — разбиваем длинные ответы
    MAX_LEN = 3800
    if len(answer) <= MAX_LEN:
        try:
            await message.answer(answer, reply_markup=kb)
        except Exception as e:
            logger.error(f"AI answer send error: {e}")
            try:
                await message.answer(answer)
            except Exception:
                await message.answer(
                    "⚠️ Ошибка отправки ответа. Попробуйте ещё раз."
                    if lang == "ru" else "⚠️ Жауапты жіберу қатесі. Қайталап көріңіз."
                )
    else:
        chunks = []
        current = ""
        ...
        for i, chunk in enumerate(chunks):
            kb_to_send = kb if i == len(chunks) - 1 else None
            try:
                await message.answer(chunk, reply_markup=kb_to_send)
            except Exception as e:
                logger.error(f"AI chunk send error: {e}")
...
async def process_file(message: Message, state: FSMContext):
    ...
    await message.answer(
        "⏳ Читаю документ, подождите..." if lang=="ru" else "⏳ Құжатты оқып жатырмын, күтіңіз...",
        ...
        if credits:
            ...
            for i, chunk in enumerate(chunks):
                try:
                    ...
                    logger.warning(f"Markdown chunk failed ({e}), retrying plain")
                    try:
                        plain = chunk.replace("*", "").replace("_", "").replace("`", "")
                        await message.answer(plain)
                    except Exception as e2:
                        logger.error(f"Plain chunk also failed: {e2}")

            # Вопрос + кнопки — отдельным сообщением
            try:
                await message.answer(q, parse_mode="Markdown", reply_markup=kb_gkb(credits, lang))
            except Exception as e:
                logger.warning(f"kb_gkb send failed ({e}), retrying plain")
                await message.answer(q, reply_markup=kb_gkb(credits, lang))
...
async def pick_banks(message: Message, state: FSMContext):
    ...
    if text.startswith("📄 Изменить ноль:") or text.startswith("📄 Нөлді өзгерту:"):
        ...
        matched = [
            cr for cr in credits
            if bank_part.lower() in _clean_bank_name(cr["bank"]).lower()
            or _clean_bank_name(cr["bank"])[:28].lower() == bank_part.lower()
        ]
        if matched:
            cr = matched[0]
    ...
    all_matched = [
        cr for cr in credits
        if clean.lower() in _clean_bank_name(cr["bank"]).lower()
        or _clean_bank_name(cr["bank"])[:38].lower() == clean.lower()
    ]
